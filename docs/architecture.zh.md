...
seam 正是替换一个提供方就能改变整个产品的原因。文件系统、普通 subprocess 与可选 PTY 提供方可以共享同一个执行世界；把所选适配器指向远程沙箱，就能把 Bash、持久终端和 LSP 一并搬过去，无需消费方专用 fork。单独划分 PTY 后，不提供持久终端的部署可以省略原生终端依赖。[subagent 提供方](subsystems/subagent.zh.md)在同一个接口之后同样千差万别，从新建一个子 agent，到把一个轮次委派给另一个产品。
