...
seam 正是替换一个 provider 就能改变整个产品的原因。文件系统、普通 subprocess 与可选 PTY provider 可以共享同一个执行世界；把所选 adapter 指向远程沙箱，就能把 Bash、persistent terminal 和 LSP 一并搬过去，无需 consumer fork。单独划分 PTY 后，不提供 persistent terminal 的部署可以省略 native terminal 依赖。[subagent provider](subsystems/subagent.md)在同一个接口之后同样千差万别，从新建一个子 agent，到把一个轮次委派给另一个产品。
