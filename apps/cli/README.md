...
The invoking directory is the default workspace root. The `web`, `ide`, and `headless` profiles auto-initialize on first use from shipped templates; `ide` adds the workspace-file and editor surface over the browser application. Any other profile must be created through `dsh plugin`.
...
Bundles named in `dsh.profile.bundles` resolve from the dsh installation first (`@deepseek-ai/dsh-base`, `@deepseek-ai/dsh-web-app`, `@deepseek-ai/dsh-ide-app`, `@deepseek-ai/dsh-headless`), then from the profile's own `node_modules`, where pnpm installs out-of-tree plugins.
