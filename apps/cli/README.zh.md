...
运行命令时所在的目录将作为默认 workspace 根目录。`web`、`ide` 和 `headless` profile 在首次使用时会从随附模板自动初始化；`ide` 会在浏览器应用上增加工作区文件与编辑器 surface。其他任何 profile 都必须通过 `dsh plugin` 创建。
...
`dsh.profile.bundles` 中列出的组合包先从 dsh 安装目录解析（`@deepseek-ai/dsh-base`、`@deepseek-ai/dsh-web-app`、`@deepseek-ai/dsh-ide-app`、`@deepseek-ai/dsh-headless`），再从 profile 自身的 `node_modules` 解析；pnpm 会将树外插件安装到该目录。
