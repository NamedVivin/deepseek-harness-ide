...
The exported `ClientTransportHooks` names the page global `__DSH_TRANSPORT__` used by the Web provider. The served app leaves it unset and gets HTTP plus WebSocket transport; a shell that owns another physical transport, such as the worker preview's `postMessage` tunnel, provides `createApiClient` and `fetch`, plus `loadBundle` when it also owns bundle bytes, without forking the carrier-neutral plugin.
