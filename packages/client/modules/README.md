...
The Node half scans enabled Loader entries for renderer-compatible `dsh.client` packages, resolves each `exports["./client"]`, hashes the built bundle into the carrier-neutral boot graph, carries package-specific `dsh.client.external` requests, and orders dynamic providers before consumers. Exactly one `ctx.clientModuleDelivery` provider supplies each row's physical URL and publishes its assets: `modules-web` owns `/plugins` plus index injection, while `modules-desktop` owns the immutable `dsh-app://` mapping. The existing `platform: "web"` declaration means browser-runtime compatible, whether the carrier is HTTP or Electron. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures. Node carriers that only validate the boot graph import the pure ESM `./manifest` subpath; `./client` is the browser lazy-CJS plugin artifact and is not a Host module.
