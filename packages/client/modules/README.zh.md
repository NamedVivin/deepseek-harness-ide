...
Node 侧会扫描已启用的 Loader 配置项以发现兼容 renderer 的 `dsh.client` 包，解析每个 `exports["./client"]`，把构建后的 bundle 哈希和包专属 `dsh.client.external` 请求写入载体中立的启动图，并把动态提供方排在消费者之前。恰好一个 `ctx.clientModuleDelivery` 提供方为每一行提供物理 URL 并发布资产：`modules-web` 持有 `/plugins` 与 index 注入，`modules-desktop` 持有不可变 `dsh-app://` 映射。现有 `platform: "web"` 声明表示兼容浏览器运行时，无论载体是 HTTP 还是 Electron。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。只需校验启动图的 Node 载体导入纯 ESM 的 `./manifest` 子入口；`./client` 是浏览器惰性 CJS 插件产物，不是 Host 模块。
