...
The owner payload is `ToolCallOwnerProps`: `callId`, `toolName`, the frozen `block`, optional `cwd` and `home`, the asynchronous `openFile(FileLocation)` action, and the plain `inspect` callback. Path summaries relativize to the session cwd first, then replace a leftover POSIX host home with `~`; `filePath` and file opening keep the authored filesystem location and optional line. The registration receives the normal session slot runtime share. It does not receive React nodes, Runtime services, or root/subcall knowledge.
