...
export interface ChatViewInjected {
  ...
  /**
   * Offer a complete presenter-owned location to internal handlers, then the
   * Host OS fallback. Relative paths resolve against the session cwd; failure
   * rejects so the chat view can show the reason and retry the same location.
   */
  openFile: (location: FileLocation) => Promise<void>
  loadOlder: () => void
