...
export function ChatView({
  ...
  const [fileOpenError, setFileOpenError] = useState<{ location: FileLocation; message: string } | null>(null)
  const [fileOpenBusy, setFileOpenBusy] = useState(false)
  // Close/retry must ignore a settlement that started before the latest
  // gesture; otherwise a cancelled in-flight refusal reopens the dialog.
  const fileOpenRequest = useRef(0)

  const requestOpenFile = useCallback((location: FileLocation): Promise<void> => {
    const path = location.path
    const id = ++fileOpenRequest.current
    setFileOpenBusy(true)
    return openFile(location).then(
      () => {
  ...
  return (
    <div className={css.root}>
      ...
      {fileOpenError !== null && (
        <FileOpenErrorDialog
          path={fileOpenError.location.path}
          message={fileOpenError.message}
          busy={fileOpenBusy}
          onClose={closeFileOpenError}
          onRetry={() => { void requestOpenFile(fileOpenError.location) }}
          t={t}
        />
      )}
    </div>
