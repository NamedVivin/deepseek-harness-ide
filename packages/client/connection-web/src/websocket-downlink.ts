...
import type { ConnectionStream, HostConnectionTransportHost } from '@deepseek-ai/dsh-client-connection'

function send(socket: WebSocket, frame: ServerRequest): Promise<void> {
...
function failureFrame(error: unknown): ServerRequest {
  return {
    type: 'server-request',
...
export class WebSocketDownlinks {
  ...
  /** @param host - carrier-neutral Host event multiplexer. */
  constructor(private readonly host: HostConnectionTransportHost) {}
  ...
  handleMux(req: IncomingMessage, socket: Duplex, head: Buffer): void {
    this.upgrade(req, socket, head, 'events.mux')
  }
  ...
  handleHost(req: IncomingMessage, socket: Duplex, head: Buffer): void {
    this.upgrade(req, socket, head, 'events.host')
  }
  ...
  private upgrade(
    ...
    this.server.handleUpgrade(req, socket, head, (websocket) => {
      ...
      const pump = this.pump(websocket, this.host.subscribe(stream, abort.signal), abort)
      this.pumps.add(pump)
