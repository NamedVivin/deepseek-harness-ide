    ...
    if (turn === 0) {
      push({
        ...
        data: {
          turn,
          step: 0,
          chunk: { type: 'usage', usage: { inputTokens: 20, outputTokens: 8 } },
        },
      })
    }
    if (withTool) {
...
function presentResult(name: string, argsRaw: string, resultText: string): ToolResultView | undefined {
  ...
  const resultCall = call as Extract<ToolCallView, { card: 'terminal' | 'diff' }>
  switch (resultCall.card) {
    case 'terminal':
    ...
    case 'diff':
      return { card: 'diff', diffs: resultCall.diffs }
  }
}
...
function foldPlan(log: readonly SessionEvent[]): { active: boolean; pending: boolean; wanted: boolean | null } {
  ...
  for (const event of log) {
    ...
    if (item.type === 'command/run' && item.data?.['name'] === 'plan') {
      const args = item.data['args'] as string
  ...
  for (const event of log) {
    switch (event.type) {
      ...
      case 'assistant/message': {
        const step = openStep as NonNullable<typeof openStep>
      ...
      case 'tool/result': {
        ...
        const dispatched = pendingCalls.get(callId) as number
...
function contextBreakdownOf(log: readonly SessionEvent[]): FixtureContextBreakdownProjection {
  ...
  for (const seq of foldSurface(log).nodes) {
    const event = log[seq] as SessionEvent
    const message = deriveEventMessage(event) as UserMessage | AssistantMessage | ToolResultMessage
    messageTokens += estimateFixtureContent(message.content) + ROLE_OVERHEAD
  }
  return {
    // The fixture does not manufacture a model request header; those buckets
    // stay empty while message pricing follows the durable surface log.
    systemTokens: 0,
    toolsTokens: 0,
    messageTokens,
  }
...
function phraseMatch(document: readonly FixtureSearchToken[], phrase: readonly string[]): FixturePhraseMatch {
  ...
  for (let start = 0; start <= document.length - phrase.length; start++) {
    ...
    count++
    if (count === 1) {
      firstStart = (document[start] as FixtureSearchToken).start
      firstEnd = (document[start + phrase.length - 1] as FixtureSearchToken).end
    }
  }
...
function backscanTodos(log: readonly SessionEvent[]): TodoItem[] | undefined {
  for (let i = log.length - 1; i >= 0; i--) {
    const event = log[i] as SessionEvent
  ...
  const api: ApiProxy = {
    sessions: {
      ...
      prompt: (request) => {
        ...
        const selection = modelSelections.get(id) as ModelSelection
        ...
        startReply(
          ...
          userText === 'render markdown'
            ...
            : userText === 'report model'
              ? (() => {
                const selection = modelSelections.get(id) as ModelSelection
                return `当前模型：${selection.provider}/${selection.model}`
