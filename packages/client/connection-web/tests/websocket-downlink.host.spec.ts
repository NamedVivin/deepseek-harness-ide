...
import type { HostConnectionTransportHost } from '@deepseek-ai/dsh-client-connection'
...
function api(mux: MuxSource, host: HostSource): HostConnectionTransportHost {
  return {
    onChannel: () => async () => {},
    invoke: () => Promise.reject(new Error('unused unary transport')),
    respond: () => Promise.reject(new Error('unused response transport')),
    subscribe(stream, signal) {
      return complete(stream === 'events.mux' ? mux(signal) : host(signal))
    },
  }
}
