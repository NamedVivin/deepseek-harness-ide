...
Host-only carrier providers in this group expose no `./client` entry, contain no `src/client` tree, and use no `staticLinked` build. The verifier retains them in the workspace declaration roster but excludes them from browser package mode and dependency classification; general workspace dependency rules still apply.
