...
// ONE register() call declares the five child slots + seats the shared store
...
describe('ui-layout client apply', () => {
  ...
  it('provides ctx.layout and registers AppFrame into root with the five child declarations', async () => {
    ...
    expect(slots.spec('shell.editor')).toEqual({ kind: 'single', scope: 'root' })
  ...
  it('injects no business face and attaches the layout actions', async () => {
    ...
    await fiber.await()
    const actions = {
      setSidebar: vi.fn(), setDetails: vi.fn(), setEditor: vi.fn(), toggleSidebar: vi.fn(), setNarrow: vi.fn(),
      setEditorRailForced: vi.fn(),
      openDetails: vi.fn(), closeDetails: vi.fn(), openEditor: vi.fn(), closeEditor: vi.fn(),
    }
  ...
  it('teardown unwinds the service, the root registration, and the child declarations', async () => {
    ...
    expect(slots.spec('shell.editor')).toBeUndefined()
