...
限定在 Workspace 内的 IDE Client 插件。它向 `shell.overlay` 贡献一个 root scope 编辑器界面，同时向 `sidebar.footer.action` 贡献一个开关；两个注册项接收 `apply` 内创建的同一个 store handle，因此隐藏 overlay 后，已打开标签、dirty buffer、保存状态与冲突恢复状态都会保留。该界面在没有 Session 时也可用，并且不会替换应用 shell 或会话 UI。
...
关闭 dirty 标签或在存在 dirty 标签时切换 Workspace，都会提供明确的保存、放弃或取消对话框。隐藏 IDE 返回会话界面不属于关闭操作，不会提示，也不会丢弃 buffer。首版的未保存 buffer 仅存在于进程内，进程崩溃时可能丢失。
...
- 隐藏 overlay 会保留临时标签状态，但崩溃恢复与持久草稿仍延期。
