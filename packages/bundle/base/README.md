...
The bundle manifest also anchors every Service Definition and shared value-imported runtime required by its provider and Consumer rows, including filesystem, shell, sandbox, subprocess, telemetry, title, spill, compaction, workflow, identity, and in-process subagent services. It carries both platform shell implementations because the shared patch selects one at runtime. A production deploy therefore carries the complete shared runtime even when a later mode bundle replaces a provider, instead of relying on workspace-only peer installation behavior.
