...
- **`listDir` / `listDirBounded`** — list one directory level in stable `name.localeCompare()` order. Each entry carries the child basename, type, resolved child target (`displayPath` under the listed directory, `targetKey` as the realpath identity), and cheap stat metadata (`version`, plus `size` for regular files). Neither opens or decodes file contents. The bounded method reads through `opendir` with one-entry buffering, stops at `maxEntries + 1`, and throws `FS_TOO_LARGE` before resolving child metadata when the complete level does not fit; it sorts and resolves only a within-limit level and never calls the unbounded method. Missing targets report `FS_NOT_FOUND`, file/special-file targets report `FS_NOT_DIRECTORY`, aborted calls report `FS_ABORTED`, permission failures report `FS_PERMISSION_DENIED`, and other listing or child metadata I/O failures report `FS_IO_ERROR`. Broken/disappeared children are returned as `other` without metadata, but permission/IO failures while resolving a child fail the whole listing with a structured `FsError`.
