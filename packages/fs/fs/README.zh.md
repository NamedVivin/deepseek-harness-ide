...
| `listDirBounded(target, { maxEntries }, signal?)` | 只有完整目录至多包含 `maxEntries` 个子项时，才返回相同的单层条目形态。每个提供方观察到第 `maxEntries + 1` 个子项就会停止并抛出 `FS_TOO_LARGE`；它绝不委托给 `listDir`，也不会把完整超限目录实体化。成功的完整结果按名称排序。 |
...
- **只有十三个原语**：没有删除、重命名/移动、复制或监视；两种列出方法都只支持一层，有界形态会返回完整层级或失败，而不是分页。递归、glob、分页和搜索仍不在范围内，见[目录列出 Agent Note](../../../.agents/notes/archived/architecture/2026-07-03-filesystem-directory-listing-seam.md)。
