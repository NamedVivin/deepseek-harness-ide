...
- **Remote identity and metadata** — relative paths resolve as POSIX paths against the caller cwd or `ctx.e2b.cwd`; GNU `realpath -mz` supplies canonical target identity without requiring the final file to exist, and ASCII/base64 plus strict NUL framing preserves newline and multibyte paths across the decoded SDK transport. `stat`, no-follow `lstat`, and `listDir` project E2B SDK metadata into the filesystem seam; the unbounded listing reuses returned metadata and resolves symbolic-link entries sequentially. `listDirBounded` never calls that SDK listing: remote GNU `find` emits name/type/size NUL records through `head -z`, stops at `maxEntries + 1`, and sends strict base64 framing to the host. The host sorts complete levels and canonicalizes symlink targets sequentially; an overflow reports `FS_TOO_LARGE`. Its lightweight entries omit the optional version and symlink size. Versions returned by other metadata operations are opaque hashes of E2B metadata plus a per-write extended attribute.
...
- **The POC targets E2B's default Linux image** — it relies on GNU `realpath`/`find`/`head`/`base64`/`chmod`, same-filesystem rename, streaming reads, and metadata extended attributes; custom templates are outside this POC.
