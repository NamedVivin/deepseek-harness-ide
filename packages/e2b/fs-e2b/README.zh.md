...
- **远程身份与元数据**：相对路径以调用方 cwd 或 `ctx.e2b.cwd` 为基准，按照 POSIX 路径解析；GNU `realpath -mz` 提供规范化目标身份，且不要求最终文件存在；ASCII/base64 加严格 NUL 分帧会在已解码的 SDK 传输中保留含换行符和多字节字符的路径。`stat`、不跟随链接的 `lstat` 和 `listDir` 会把 E2B SDK 元数据投影到文件系统 seam；无界列表复用已返回的元数据，并依次解析符号链接条目。`listDirBounded` 绝不调用该 SDK 列表：远程 GNU `find` 通过 `head -z` 发出名称／类型／大小 NUL 记录，在第 `maxEntries + 1` 个条目处停止，再以严格 base64 分帧传给宿主。完整层级会在宿主排序，符号链接目标会依次规范化，超限报告 `FS_TOO_LARGE`；这种轻量条目会省略可选版本和符号链接大小。其他元数据操作返回的版本是 E2B 元数据与每次写入设置的扩展属性所组成的不透明哈希。
...
- **该 POC 面向 E2B 默认 Linux 镜像**：它依赖 GNU `realpath`／`find`／`head`／`base64`／`chmod`、同一文件系统内的 rename、流式读取和元数据扩展属性；自定义模板不在该 POC 范围内。
