...
describe('E2B subprocess terminal service', () => {
  async function service(fake = new FakeTerminalSandbox()): Promise<{
    ...
    const fiber = await ctx.plugin(E2BSubprocessPtyRuntime)
  ...
  it('rejects a non-positive poll cadence at load', async () => {
    ...
    await expect(ctx.plugin(E2BSubprocessPtyRuntime, { pollMs: 0 }))
      .rejects.toThrow('pollMs must be a positive safe integer')
    const explicit = await ctx.plugin(E2BSubprocessPtyRuntime, { pollMs: 5 })
    await explicit.dispose()
  })

  it('owns live terminals through service disposal', async () => {
    const { ctx, fiber, fake } = await service()
    const terminal = await ctx.subprocessPty.spawnTerminal(spec({ signal: new AbortController().signal }))
    await fiber.dispose()
    await expect(terminal.done).resolves.toEqual({ exitCode: null, signal: 'SIGTERM' })
    expect(fake.handle.disconnects).toBe(1)
  })
  ...
  it('joins and rejects terminal setup that completes during service disposal', async () => {
    ...
    const subprocess = ctx.subprocessPty
    const spawning = ctx.subprocessPty.spawnTerminal(spec())
  ...
  it('releases naturally settled terminals and validates terminal requests', async () => {
    ...
    for (const request of [
      ...
      await expect(ctx.subprocessPty.spawnTerminal(request)).rejects.toThrow()
    }

    fake.groups = []
    const terminal = await ctx.subprocessPty.spawnTerminal(spec())
    fake.handle.succeed(0)
    await terminal.done
    await terminal.terminate()
  ...
  it('contains a failed automatic terminal release until service disposal retries it', async () => {
    ...
    const terminal = await (fiber.ctx).subprocessPty.spawnTerminal(spec({ graceMs: 1 }))
    fake.handle.succeed(0)
    await terminal.done
  ...
  it('contains an immediate automatic terminal release rejection before disposal retries it', async () => {
    const { fiber, fake } = await service()
    fake.groups = []
    const terminal = await (fiber.ctx).subprocessPty.spawnTerminal(spec())
  ...
  it.each([
    [[new Error('single teardown failure')]],
    [[new Error('first teardown failure'), new Error('second teardown failure')]],
  ])('reports terminal teardown failures after joining all live handles %#', async (failures) => {
    ...
    if (failures.length === 1) expect(errors[0]).toBe(failures[0])
    else expect(errors[0]).toMatchObject({ errors: failures, message: 'subprocess-pty-e2b: teardown failed' })
  })
})
