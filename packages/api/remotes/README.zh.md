...
`createApiRemoteAgentResolver()` 会复用 live Agent、恢复普通冷会话、对并发恢复去重、保留 subagent ownership fence，并为 Typert `agent` 和 `session` lookup 配置同一个 resolver。所属 Host 提供 Agent 默认值与 scope setup；`mapResumeFailure` 可以保留解析或挂载该 setup 时出现的已识别 Host policy 拒绝，未识别的失败仍归为 `internal`。标准 Web API Proxy 把返回的 resolver 用于旧方法，使已迁移与未迁移方法共用同一份身份策略实现。

当前 Client 组合挂载 Goal Remote 贡献、只读 Host 插件清单贡献（`pluginInventory/list`），以及有界工作区文件贡献（`workspaceFiles/list`、`read`、`save` 和 `resolveLocation`）。该组合卸载时，Cordis effect 的所有权机制会撤回所有贡献；`@deepseek-ai/dsh-api-gateway/client` 负责描述符校验、可追踪 namespace Service、直接与作用域方法、调用与取消。Client 入口通过 Cordis 消费共享的 `TypertClientRemote` 接口，不导入具体 Gateway；它只以 type-only 形式重新导出 Gateway Client face 的声明合并与工作区文件 payload 词汇，因此消费端经由本外观取到这些类型或转发事件词汇时，运行时不会多出一条通往 Host 实现的边。
