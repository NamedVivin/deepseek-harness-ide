...
`createApiRemoteAgentResolver()` reuses live Agents, resumes ordinary cold sessions, deduplicates concurrent resumes, preserves the subagent ownership fence, and configures the same resolver for Typert `agent` and `session` lookups. The owning Host supplies Agent defaults and scope setup; `mapResumeFailure` may preserve a recognized Host policy refusal raised while resolving or mounting that setup, while every unrecognized failure remains `internal`. The standard Web API Proxy uses the returned resolver for legacy methods, so migrated and unmigrated methods share one identity policy implementation.

The current Client assembly mounts the Goal Remote contribution, the read-only Host plugin inventory contribution (`pluginInventory/list`), and the bounded workspace-file contribution (`workspaceFiles/list`, `read`, `save`, and `resolveLocation`). Cordis effect ownership withdraws every contribution when this assembly unloads, while `@deepseek-ai/dsh-api-gateway/client` owns descriptor validation, traced namespace Services, direct and scoped methods, invocation, and cancellation. The Client entry consumes the shared `TypertClientRemote` interface through Cordis and does not import the concrete Gateway. It re-exports the Gateway Client face's declaration merges and workspace-file payload vocabulary type-only, so a consumer reaching those types or the forwarded-event vocabulary through this facade gains no runtime edge to a Host implementation.
