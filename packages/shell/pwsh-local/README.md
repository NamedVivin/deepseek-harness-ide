...
- **A post-creation process-observation failure note is single-delivery** — after `start()` has resolved, a rejected subprocess `done` settles the `ShellProcess` as `killed`; when no retained stderr exists, the executor injects `process failed: …` into exactly one `readOutput()` delta. A reader that discards that delta cannot recover it.
