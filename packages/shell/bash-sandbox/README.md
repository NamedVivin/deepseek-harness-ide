...
- **A runner failure after process publication has no immediate error channel** — it is recorded on the settled process and surfaces when the caller reads the generic task with `job_output`; a runner-attributable subprocess creation rejection instead rejects `start()` before publishing a `ShellProcess`.
