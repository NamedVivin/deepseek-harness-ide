...
| `start(spec)` | Background execution. Returns a Promise that resolves to a task-free `ShellProcess` only after the subprocess is created and owned; creation failures reject before any process handle is published. **No timeout applies** after creation. The caller may adapt the handle into `ctx.jobs`. |
