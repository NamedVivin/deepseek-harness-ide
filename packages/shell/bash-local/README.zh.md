...
- **创建后进程观察失败提示只交付一次**：`start()` resolve 后，如果 subprocess 的 `done` reject，`ShellProcess` 会以 `killed` 结算；如果没有保留的 stderr，执行器会把 `process failed: …` 注入恰好一个 `readOutput()` 增量。丢弃了该增量的读取方无法再恢复它。
