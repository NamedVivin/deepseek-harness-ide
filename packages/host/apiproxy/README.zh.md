...
preset 准入先于新 cwd 创建、Agent create/resume、冷 presenter 组装、fork 子 Agent 创建和 preset 选择。桌面 provider 的拒绝会在所有这些入口上以 `desktop-preset-unsupported` 返回，并携带尝试的 preset、支持的 preset、preset service 操作和稳定原因；被拒 preset 不会挂载，被拒选择也不会追加 `agent-preset/selected` 事件。冷转录遇到未知或损坏 preset 时仍保留 Web 行为，改用全局 presenter view 提供内容。
