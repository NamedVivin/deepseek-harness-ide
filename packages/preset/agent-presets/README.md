...
- `ctx.agentPresets.registerAdmission(contribution): () => void` Add synchronous, operation-aware admission policy for the calling Cordis effect. The returned exact disposer removes the contribution and starts a new policy generation.
...
- `ctx.agentPresets.composeFrom(agentCtx, parentCtx): string | undefined` Join one agent to the standing composition another already runs on, returning the preset id joined — `undefined` when the parent joined none, which is the rosterless deployment and not an error. A bind rather than a mount, so it stays synchronous; it rejects a policy refusal, a standing generation admitted by an obsolete policy, and caller errors such as an unscoped or already-joined agent.
...
No admission contribution means allow, preserving rosterless, Web, and headless compositions. A contribution receives `{ operation, presetId }`, where `operation` is `resolve`, `mount`, `recompose`, `standingKeyFor`, or `composeFrom`, and either returns `undefined` or a refusal `{ code, reason, details? }`. The first refusal throws `PresetAdmissionError`; its stable `metadata` contains `operation`, `presetId`, `code`, `reason`, and optional `details`, so a transport or UI does not need to parse the message.

`registerAdmission()` records the contribution through the caller's Cordis effect. Registration and disposal each rotate the policy generation. Every admitted public operation carries an opaque, single-use proof, and only a `mount`, `recompose`, or `standingKeyFor` proof can be claimed by the low-level mount. Claiming happens before the preset subtree is plugged, so a caller holding an `AgentPreset` cannot bypass policy and a refusal registers no provider or subtree. The proof stored on `StandingMount` is the same proof stored on its public `PresetMount` record, and runtime identity—not a structural TypeScript cast—decides whether it is genuine.

Existing agents keep running when policy changes; their standing subtree is not torn down. That subtree's proof becomes obsolete for new work, so the next mount-like operation creates a generation admitted by the current contributions, while `composeFrom()` refuses to bind a new child to the obsolete generation. This preserves a running conversation without letting old mount authority admit another agent.
