...
`start(request)` accepts only a non-empty sequence of text blocks and derives the child cwd from the parent Session. The pinned SDK exposes only a synchronous `spawnClaudeCodeProcess` hook, while [`dsh-subprocess`](../../subprocess/subprocess/README.md) publishes handles asynchronously. The provider therefore asks `query()` to compose its spawn request once with a capture hook that aborts before a Query is published, awaits exactly one real subprocess spawn, then calls `query()` again with the ready managed process. The second hook must be invoked exactly once, and its command, arguments, cwd, and environment must match the captured request; missing capture, unexpected Query publication, changed parameters, or repeated real hooks fail explicitly. The run publishes only after the real Query claims the positive-PID, provider-owned CLI handle. A failure or cancellation before publication closes any Query, terminates any acquired process tree, waits for it to exit, and rejects `start()`.
