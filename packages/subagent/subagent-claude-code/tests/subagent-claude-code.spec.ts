...
describe('task admission and package contracts', () => {
  ...
  it('keeps named instances, runs, and HMR ownership isolated', async () => {
    ...
    expect(safeChild.terminate).toHaveBeenCalledTimes(2)
    expect(bypassChild.terminate).toHaveBeenCalledTimes(2)
  ...
  it('starts through the registered provider with its resolved config and diagnostics', async () => {
    ...
    expect(errorCause(warn.mock.calls[0]?.[1] as unknown)?.message)
      .toBe('subagent-claude-code: SDK spawn-request capture failed')
    expect(errorCause(errorCause(warn.mock.calls[0]?.[1] as unknown))?.message)
...
describe('query options and result mapping', () => {
  ...
  it.each(CLAUDE_CODE_PERMISSION_MODES)(
    ...
    (permissionMode) => {
      ...
      }, new AbortController(), () => new ManagedClaudeCodeProcess(child.handle), () => {})
...
describe('run publication, cancellation, and settlement', () => {
  ...
  it('starts one real process and fails explicitly when the SDK launch request changes', async () => {
    ...
    const changedRequest = startClaudeCodeRun(request(), {
      cwd: '/workspace',
      permissionMode: DEFAULT_CLAUDE_CODE_PERMISSION_MODE,
      env: {},
      disposeGraceMs: 5,
    ...
    await expect(changedRequest)
      .rejects.toThrow(expectedFailureDiagnostic('query-start', 'unknown'))
    await expect(changedRequest).rejects.toHaveProperty(
      'cause.message',
      'subagent-claude-code: SDK spawn request changed between capture and launch (args)',
    )
  ...
  it('omits captured diagnostics on success and isolates concurrent runs', async () => {
    ...
    const spec: ClaudeCodeRunSpec = {
      ...
      env: {},
      disposeGraceMs: 5,
      spawn: async () => children[childIndex++]!.handle,
    }
  ...
  it('rejects pre-abort and every incomplete startup transaction', async () => {
    ...
    await expect(noChild)
      .rejects.toThrow(expectedFailureDiagnostic('query-start', 'unknown'))
    const noChildError = await noChild.then(
      () => undefined,
      (error: unknown) => error,
    )
    expect(errorCause(noChildError)).toBeInstanceOf(AggregateError)
    ...
    const cancelledCleanupFailure = startClaudeCodeRun(
      ...
      {
        ...unused.spec,
        spawn: async () => cleanupFailedChild.handle,
      },
    )
    ...
    expect(errorCause(queryFailureOnError.mock.calls[0]?.[0])?.message)
      .toBe('subagent-claude-code: SDK spawn-request capture failed')
    expect(errorCause(errorCause(queryFailureOnError.mock.calls[0]?.[0]))?.message)
    ...
    let cleanupRaceSdkCalls = 0
    queryMock.mockImplementation(({ options }) => {
      cleanupRaceSdkCalls += 1
      options.spawnClaudeCodeProcess!(sdkSpawnOptions())
      if (cleanupRaceSdkCalls === 2) {
    ...
    const cleanupRace = startClaudeCodeRun(
      ...
      {
        ...unused.spec,
        spawn: async () => cleanupRaceChild.handle,
      },
    )
    await nextTask()
    ...
    const spawnFailureOnError = vi.fn<
      NonNullable<ClaudeCodeRunSpec['onError']>
    >()
    const failedStartup = startClaudeCodeRun(request(), {
      ...failed.spec,
      spawn: async () => { throw spawnError },
      onError: spawnFailureOnError,
    })
    ...
    expect(spawnFailureOnError).toHaveBeenCalledWith(expect.any(Error), 'error')
    expect(failed.close).not.toHaveBeenCalled()

    const pendingAbort = new AbortController()
    const spawnEntered = Promise.withResolvers<undefined>()
    const pending = fakeRun()
    const pendingStartup = startClaudeCodeRun(
      request(undefined, pendingAbort.signal),
      {
        ...pending.spec,
        spawn: spawnSpec => new Promise((_resolve, reject) => {
          spawnEntered.resolve(undefined)
          spawnSpec.signal!.addEventListener('abort', () => {
            const reason: unknown = spawnSpec.signal!.reason
    ...
    pendingAbort.abort(new Error('startup cancelled'))
    await expect(pendingStartup).rejects.toThrow('aborted before SDK startup')
    ...
    let liveCleanupSdkCalls = 0
    queryMock.mockImplementation(({ options }) => {
      liveCleanupSdkCalls += 1
      options.spawnClaudeCodeProcess!(sdkSpawnOptions())
      if (liveCleanupSdkCalls === 2) throw constructionError
      return queryFrom([])
    })
    const liveCleanupFailure = startClaudeCodeRun(request(), {
      ...unused.spec,
      spawn: async () => liveChildCleanupFailure.handle,
    })
...
describe('query and process disposal', () => {
  ...
  it('reports close and tree-wait failures without skipping cleanup', async () => {
    ...
    const bothFailure = disposeClaudeCodeChild(
      { close: () => { throw new Error('close boom') } },
      both.handle,
    )
    const bothError = await bothFailure.then(
      () => undefined,
      (error: unknown) => error,
    )
    expect(errorCause(bothError)).toBeInstanceOf(AggregateError)
  })
})
