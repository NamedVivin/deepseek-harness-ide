...
- `start(spec): Promise<JobId>` 验证已附加的任务控制器、spec、确切且仍存活的 owner、可选的正数 `outputLimitBytes`，以及 Service Provider 所拥有的准入策略，然后只调用生产方的异步 `run(signal)` 一次。注册表会在等待就绪 `JobHooks` 时预留准入名额；owner 或服务 teardown 会中止设置信号并等待未发布工作。预检或启动方拒绝时都不会生成 job id 或注册工作。如果钩子就绪后 teardown 胜出，注册表会先取消并等待这些资源，再拒绝启动，而不会发布临时记录。
...
实现还必须兑现约定的生命周期语义：注册的存续期长于生产方 fiber 与控制器 fiber，挂起的启动会预留容量但不会出现在读取结果中，owner 释放和服务释放会取消并等待未发布设置与存活工作，结算遵循首次结果优先（一条终止记录、一轮异常受到隔离的监听器通知，然后释放等待方）。
...
- **约定是进程内的**：`JobStart.run(signal)` 传入回调、设置信号和确切的 `Agent` 对象；持久化或跨进程后端必须先重塑身份、重启、所有权与观察语义，才能实现此 seam。
