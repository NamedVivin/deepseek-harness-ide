...
子进程 seam（`ctx.subprocess`）是一个执行世界的普通进程部分。抽象的 `SubprocessRuntime` 公开可执行文件查找与受管 `spawn`；其词汇涵盖原始／收集式 stdio、进程句柄、退出事实、进程树清理，以及受管的 `DSH_*` 环境命名空间。本地实现位于 [`dsh-subprocess-local`](../subprocess-local/README.zh.md)。真实终端会话使用独立且可选的 [`ctx.subprocessPty`](../subprocess-pty/README.zh.md) 能力。

## 约定

- `spawn(spec)` 返回一个 Promise，且只在提供方取得正进程 ID 并拥有该进程树后 resolve。创建或所有权建立失败会直接 reject，不发布句柄。发布后，`done` 在进程关闭时以退出事实 resolve（`SubprocessOutcome` 不携带输出，也不携带原因分类），仅在后续进程观察失败时 reject。
...
- `scrubbedParentEnv()` / `SENSITIVE_ENV_PATTERN` 是唯一一份共享的环境清理定义：环境中形似凭据的名称与 `DSH_*` 名称都会被丢弃，显式 `env` 在清除之后合并。本地的普通进程与 PTY provider 都应用该定义；拥有自身 spawn 的 SDK 管理传输可直接导入它。
...
- **SDK 传输需要稳定的自定义 spawn 请求**：带同步自定义 spawn 钩子的传输可以捕获确定性的请求、等待本服务，再围绕就绪句柄创建真实传输；`dsh-subagent-claude-code` 会锁定并验证这项两阶段适配。没有自定义钩子或稳定请求组合的 SDK 仍位于本服务之外，但仍可导入 `scrubbedParentEnv`，使环境策略保持单一来源。
