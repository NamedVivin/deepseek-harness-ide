...
子进程 seam（`ctx.subprocess`）是一个执行世界的普通进程部分。抽象的 `SubprocessRuntime` 公开可执行文件查找与受管 `spawn`；其词汇涵盖原始／收集式 stdio、进程句柄、退出事实、进程树清理，以及受管的 `DSH_*` 环境命名空间。本地实现位于 [`dsh-subprocess-local`](../subprocess-local/README.md)。真实终端会话使用独立且可选的 [`ctx.subprocessPty`](../subprocess-pty/README.md) 能力。
