...
describe('package modes', () => {
  ...
  it('rejects browser packages with both modes or neither mode and accepts a Host-only carrier', () => {
    const both = pkg('both', { staticLinked: true })
    const neither = pkg('neither', { browserEntry: false, dynamic: false })
    const carrier = pkg('modules-web', { browserEntry: false, browserSource: false, dynamic: false })
    const found = collectClientPackageViolations(facts([both, neither, carrier]))
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + 'packages/client/modules-web/src/index.ts',
    ])
  })
})
