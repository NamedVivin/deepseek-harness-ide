...
const PARSER_PRELOAD_SOURCE = 'packages/client/modules-web/src/index.ts'
...
/** One package directly under packages/client, including Host-only carrier providers. */
export interface ClientPackage extends ClientDeclaration {
  /** Whether the package exports include a browser `./client` entry. */
  readonly browserEntry: boolean
  /** Whether production sources include a browser `src/client` tree. */
...
export interface ClientPackageFacts {
  /** Packages directly under packages/client, including Host-only carrier providers. */
...
export function fixClientPackageManifests(root: string, facts: ClientPackageFacts): string[] {
  ...
  for (const pkg of facts.packages) {
    if (!isBrowserPackage(pkg)) continue
...
function collectModeViolations(facts: ClientPackageFacts): string[] {
  ...
  for (const pkg of facts.packages) {
    ...
    } else if (!pkg.dynamic && !pkg.staticLinked && isBrowserPackage(pkg)) {
      violations.push(
...
async function readFacts(root: string): Promise<ClientPackageFacts> {
  ...
  for (const manifestPath of globSync(CLIENT_MANIFEST_GLOB, { cwd: root }).map(normalizePath).sort()) {
    ...
    for (const sourceFile of project.sourceFiles()) {
      ...
      if (file.startsWith(sourcePrefix + 'client/')) browserSource = true
    ...
    packages.push({
      ...declaration,
      browserEntry: isRecord(manifest.exports) && Object.hasOwn(manifest.exports, './client'),
      browserSource,
...
function isBrowserPackage(pkg: ClientPackage): boolean {
  return pkg.dynamic || pkg.staticLinked || pkg.browserEntry || pkg.browserSource
}
...
async function main(): Promise<void> {
  ...
  const browserPackages = facts.packages.filter(isBrowserPackage)
  const dynamic = browserPackages.filter(pkg => pkg.dynamic).length
  const hostOnly = facts.packages.length - browserPackages.length
  const requests = facts.declarations.reduce((total, pkg) => total + pkg.external.length, 0)
  console.log(
    GATE + ': ' + String(browserPackages.length) + ' browser packages (' + String(dynamic) + ' dynamic, '
    + String(browserPackages.length - dynamic) + ' statically linked) and ' + String(hostOnly)
    + ' Host-only carrier package(s) satisfy dependency and module-request rules; '
