...
## The data object layer (`packages/client/runtime/src/client/`)
...
Domain implementation files never import a sibling domain. Shared APIs route through `contract/`; package-private implementation used by several domains lives in top-level non-assembly modules, which may import `contract/` and top-level peers but never a domain. `scripts/verify-client-domain-graph.ts` enforces the layering (contract and top-level shared modules=0, domains=1, apply/index=2; imports may only point at levels ≤ own; sibling-domain edges fail). Tool presentation is already a separate `ui-tool` package and reaches chat and details only through the slots ui-conversation declares.
