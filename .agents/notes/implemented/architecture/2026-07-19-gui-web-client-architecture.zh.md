...
## 数据对象层（`packages/client/runtime/src/client/`）
...
各领域实现文件不 import 兄弟领域。共享 API 统一经过 `contract/`；多个领域共用的包内实现放在顶层非组装模块中，这些模块可 import `contract/` 与其他顶层模块，但不能反向 import 领域目录。`scripts/verify-client-domain-graph.ts` 把守分层（contract 与顶层共享模块 = 0、domain = 1、apply/index = 2；import 只准指向不高于自身的层级；兄弟领域依赖会失败）。Tool 展示已经拆为独立 `ui-tool` 包，只通过 ui-conversation 声明的 slot 到达 chat 与 details。
