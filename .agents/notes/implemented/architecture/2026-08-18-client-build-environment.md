...
The Vite static shell and shared tsdown dynamic bundles receive the same string for a given `DSH_CLIENT_*` build-process variable. An unset static property read evaluates to `undefined`; non-`DSH_CLIENT_*` values cannot enter browser artifacts through this mechanism, and business code cannot enumerate the build process environment. Every complete build carries its short source revision as public display metadata. CI build gates and the artifact-consuming `check:all` aggregate select the official profile without exposing its public values to source tests or unrelated workflow steps. `check:all` uses that single complete root build for built snapshots and does not schedule a partial Web rebuild that could replace its recorded artifacts. npm packing and built Web tests verify the recorded environment and current artifact digest, so a default build followed by an official pack request, a partial rebuild, or modified output fails before consumption.
