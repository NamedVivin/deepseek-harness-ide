# Agent Note: Web client Agent-scope parity model and the provisioning channel (scope.ts / blank reuse / provide)
...
The runtime's `scope.ts` matches the host's `dsh-scope` at the mechanism layer (fiber + tag + filter; no value import: the host package carries the scoped-events `Events` merge, which would collide with the Context merge inside the client program):
